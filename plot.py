...
def plot_xrd(patterns, titles, wavelength, experimental_data=None, opacity=0.9):
    ...
    fig.update_layout(
        ...
        font=dict(family="Open Sans", size=24),
        xaxis=dict(
            title=dict(text="diffraction angle, 2<i>θ</i>", font=dict(family="Open Sans", size=24)),
            range=[x_min, x_max],
            tickmode='array',
            tickvals=big_ticks,
        ...
        yaxis=dict(
            title=dict(text="intensity, a.u.", font=dict(family="Open Sans", size=24)),
            range=[0, 105],
            gridcolor='lightgray',
            gridwidth=1,
            tickfont=dict(family="Open Sans", size=24)
        ),
        shapes=tick_shapes,
